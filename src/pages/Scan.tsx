...
export default function Scan() {
  ...
  const [activeTab, setActiveTab] = useState<"folders" | "processes">("folders");
  ...
  useEffect(() => {
    const unlisten1 = listen<ExeEntry>("scan:entry", async (event) => {
      ...
      setResults((prev) => [
        ...prev,
        { ...entry, selected: !exists, alreadyAdded: exists, customName: cleanName },
      ]);
    });
    ...
    return () => {
      Promise.all([unlisten1, unlisten3]).then((fns) => fns.forEach((fn) => fn()));
    };
  }, []);
  ...
  const loadProcesses = async () => {
    ...
    try {
      ...
      if (procs.length === 0) {
        setError("nvidia-smi не вернул процессов. Проверьте, установлены ли драйвера NVIDIA и запущена ли игра.");
      }

      const procsWithStatus = await Promise.all(procs.map(async (p) => {
  ...
  const toggleSelect = (listType: "folders" | "processes", path: string) => {
    ...
    setter((prev) =>
      prev.map((r) =>
        r.path === path && !r.alreadyAdded ? { ...r, selected: !r.selected } : r
      )
    );
  };
  ...
  const selectAll = (listType: "folders" | "processes") => {
    const setter = listType === "folders" ? setResults : setProcesses;
    setter((prev) => prev.map((r) => (!r.alreadyAdded ? { ...r, selected: true } : r)));
  };
  ...
  const updateName = (listType: "folders" | "processes", path: string, name: string) => {
    const setter = listType === "folders" ? setResults : setProcesses;
    setter((prev) => prev.map((r) => (r.path === path ? { ...r, customName: name } : r)));
  };
  ...
  const selectedCount = currentList.filter((r) => r.selected && !r.alreadyAdded).length;
  ...
  return (
    <div className="p-3 sm:p-6 h-full flex flex-col max-w-5xl mx-auto w-full">
      ...
      <div className="mb-4 sm:mb-6">
        <h1 className="text-xl sm:text-2xl font-bold tracking-tight">Добавление игр</h1>
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
        {activeTab === "folders" ? (
          scanning ? (
            <>
              <Button variant="destructive" onClick={cancelScan} className="sm:w-auto">
              ...
              <div className="flex-1 flex flex-col gap-2">
                 <div className="flex justify-between text-[10px] sm:text-sm">
                    ...
                    <span className="text-muted-foreground">Найдено: {results.length}</span>
                 </div>
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <Button onClick={loadProcesses} disabled={loadingProcesses} className="gap-2 flex-1 sm:flex-none">
              {loadingProcesses ? <Loader2 className="w-4 h-4 animate-spin" /> : <RefreshCw className="w-4 h-4" />}
              Обновить список
            </Button>
          </div>
        )}
        ...
        <div className="flex flex-col gap-3 mb-4">
          ...
          <div className="flex items-center justify-between gap-2 overflow-x-auto no-scrollbar pb-1">
                ...
                <div className="flex items-center bg-muted rounded-md p-0.5">
                  <button
                    ...
                    className={cn(
                      ...
                      sortBy === "cpu" ? "bg-background shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground"
                    )}
                  >
                    ЦП
                  </button>
                  ...
                  >
                    <Loader2 className={cn("w-3 h-3", loadingProcesses && "animate-spin")} />
                  </button>
                </div>
              )}
            ...
            <div className="flex items-center gap-1.5 flex-shrink-0">
              <Button variant="outline" size="sm" onClick={() => selectAll(activeTab)} className="h-7 text-[10px] px-2 sm:h-8 sm:text-xs sm:px-3">
                Все
              </Button>
              <Button variant="outline" size="sm" onClick={() => deselectAll(activeTab)} className="h-7 text-[10px] px-2 sm:h-8 sm:text-xs sm:px-3">
                Сброс
              </Button>
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col min-h-0 min-w-0">
        {error ? (
          <div className="flex-1 border border-destructive/20 bg-destructive/5 rounded-lg p-6 flex flex-col items-center justify-center text-center">
            ...
            <h3 className="text-lg font-semibold text-destructive mb-2">Произошла ошибка</h3>
            <p className="text-sm text-muted-foreground max-w-md break-all font-mono bg-background/50 p-3 rounded border">
              {error}
            </p>
            <Button variant="outline" onClick={loadProcesses} className="mt-4 gap-2">
           ...
           <div className="flex-1 border rounded-lg flex flex-col items-center justify-center p-4">
              ...
              <p className="text-xs sm:text-sm text-muted-foreground">Получение списка процессов...</p>
           </div>
                ...
                >
                    ...
                    <div className="flex items-center gap-2">
                      {result.alreadyAdded ? (
                        ...
                        <input
                          ...
                          onChange={(e) => updateName(activeTab, result.path, e.target.value)}
                  ...
                  <div className="flex flex-col items-end gap-1 flex-shrink-0 ml-auto pl-2">
                    <div className={cn(
                      ...
                      <span className="text-[9px] font-bold uppercase tracking-tighter opacity-70">CPU</span>
      ...
      {selectedCount > 0 && (
        <div className="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t flex flex-col sm:flex-row gap-3 justify-between items-center bg-background/80 backdrop-blur-sm sticky bottom-0">
          <div className="text-xs sm:text-sm">
            Выбрано <span className="font-bold text-primary">{selectedCount}</span> {selectedCount === 1 ? "игра" : "игр"}
          </div>
          <Button
